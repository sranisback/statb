...
class StatBBController extends Controller
{
    ...
    {	
		...
		$teams = $this->getDoctrine()->getRepository(Teams::class)->findBy(array(), array('name' => 'ASC'));
    ...
    public function show_user_teams()
    {
		...
		$coachTeam = $this->getDoctrine()->getRepository(Teams::class)->findBy(array('ownedByCoach'=>$this->getUser()));
		...
		foreach($coachTeam as $number=>$team) 
		{
			...
			if($team->getYear() < 2)
			{
				unset($coachTeam[$number]);
			}
			else
			{
				$matches1 = $this->getDoctrine()->getRepository(Matches::class)->findBy(array('team1'=>$team->getTeamId()),array('dateCreated'=>'DESC'));
				$matches2 = $this->getDoctrine()->getRepository(Matches::class)->findBy(array('team2'=>$team->getTeamId()),array('dateCreated'=>'DESC'));
				...
				{
					if(($team == $match->getTeam1() && $match->getTeam1Score() > $match->getTeam2Score()) || ($team == $match->getTeam2() && $match->getTeam1Score() < $match->getTeam2Score()) )
					{
						$win++;
					}
					elseif(($team == $match->getTeam1() && $match->getTeam1Score() < $match->getTeam2Score()) || ($team == $match->getTeam2() && $match->getTeam1Score() > $match->getTeam2Score()) )
					{
						$loss++;
					}
					elseif(($team == $match->getTeam1() && $match->getTeam1Score() == $match->getTeam2Score()) || ($team == $match->getTeam2() && $match->getTeam1Score() == $match->getTeam2Score()) )
    ...
    public function show_team($id,$type)
    {
		...
		$players = $this->getDoctrine()->getRepository(Players::class)->findBy(array('ownedByTeam'=>$id), array('nr' => 'ASC'));
		...
		foreach($players as $number=>$player)
		{
			...
			$supcomp = $this->getDoctrine()->getRepository(PlayersSkills::class)->findBy(['fPid' =>$player->getPlayerId()]);
		...
		if($type == "modal")
		{
			return $this->render('statbb/team_modal.html.twig',array('players'=>$players,'team'=>$team, 'pdata'=>$pdata,'tdata'=>$tdata));
		}
		else
		{
			return $this->render('statbb/team.html.twig',array('players'=>$players,'last_username' => $lastUsername,'team'=>$team, 'pdata'=>$pdata,'tdata'=>$tdata));
		}
    ...
    public function show_player($id,$type)
    {
		...
		foreach($mdata as $game)
		{
			...
			if($game->getCp()>0 || $game->getTd()>0 || $game->getIntcpt() >0 || ($game->getBh() + $game->getSi() + $game->getKi())>0 || $game->getMvp()>0 || $game->getAgg()>0)
			{
		...
		if($type == "modal")
		{
			return $this->render('statbb/player_modal.html.twig',array('player'=>$player,'pdata'=>$pdata,'matches' =>$matches,'mdata'=>$msdata,'last_username' => $lastUsername,));
		}
		else
		{
			return $this->render('statbb/player.html.twig',array('player'=>$player,'pdata'=>$pdata,'matches' =>$matches,'mdata'=>$msdata,'last_username' => $lastUsername,));
		}
    ...
    */
	public function class_gen($limit)
	{
		...
		return $this->render('statbb/classement.html.twig',array('classement' =>$classement,'limit'=>$limit));
	}
    ...
    */
	public function Sclass($type,$teamorplayer,$limit)
	{
		...
		$matches_data = $this->getDoctrine()->getRepository(MatchData::class)->Sclassement($setting->getValue(),$type,$teamorplayer,$limit);
		...
		if($teamorplayer == 'player')
		{
			...
			return $this->render('statbb/Spclassement.html.twig',array('players' =>$matches_data, 'title'=>$title, 'class'=>$class, 'type'=>$type,'teamorplayer'=>$teamorplayer, 'limit' => $limit));
		...
		else
		{
			...
			return $this->render('statbb/Stclassement.html.twig',array('teams' =>$matches_data, 'title'=>$title, 'class'=>$class, 'type'=>$type,'teamorplayer'=>$teamorplayer, 'limit' => $limit));
    ...
    */
	public function Totalcas()
	{
		...
		{
			if($match->getTeam1()->getYear() !=  $setting->getValue() || $match->getTeam2()->getYear() !=  $setting->getValue())
			{
    ...
    */
	public function create_team($teamname,$coachid,$raceid)
	{
		...
		$team->setOwnedByCoach($this->getDoctrine()->getRepository(Coaches::class)->findOneBy(array('coachId'=>$coachid)));
	...
	public function calculateTV($team)
	{
		...
		$players = $this->getDoctrine()->getRepository(Players::class)->findBy(array('ownedByTeam'=>$team->getTeamId()), array('nr' => 'ASC'));
    ...
    public function player_adder($raceId,$teamId)
    {
		...
		return $this->render('statbb/playeradder.html.twig',['playerpositions'=>$playerpositions,'teamId'=>$teamId]);
	}
	...
	*/
	public function add_player($posId,$teamId)
	{
		...
		{
			$allPlayerByPos  = $this->getDoctrine()->getRepository(Players::class)->findBy(['fPos' =>$position,'ownedByTeam'=>$team]);
			...
			{	
				...
				$allPlayersTeam  = $this->getDoctrine()->getRepository(Players::class)->findBy(['ownedByTeam'=>$team],['nr' => 'ASC']);
				...
				$html = $this->render('statbb/lineteamsheet.html.twig',['position'=>$position,'player'=>$player,'skill'=>$listskill])->getContent();
				$reponse = "ok";
			}
			else
			{
	...
	*/
	public function remPlayer($playerId)
	{
		...
		$matchest1 = $this->getDoctrine()->getRepository(Matches::class)->findBy(['team1'=>$player->getOwnedByTeam()->getTeamId()]);
		
		$matchest2 = $this->getDoctrine()->getRepository(Matches::class)->findBy(['team2'=>$player->getOwnedByTeam()->getTeamId()]);
		
		if(count($matchest1)==0 || count($matchest2)==0)
		{
			
			$player->getOwnedByTeam()->setTreasury($player->getOwnedByTeam()->getTreasury()+ $player->getFPos()->getCost());
    ...
    */
    ...
    public function dropdownTeams($nbr)
    {
		...
		$teams  = $this->getDoctrine()->getRepository(Teams::class)->findBy(['retired'=>0,'year'=>$setting->getValue()],['name' => 'ASC']);
    ...
    */
    ...
    public function dropdownPlayer($teamId,$nbr)
    {
		...
		$players  = $this->getDoctrine()->getRepository(Players::class)->findBy(['ownedByTeam'=>$teamId],['nr' => 'ASC']);
		...
		$response = ['html'=>$this->renderView('statbb/dropdownplayers.html.twig',['players'=>$players,'teamId'=>$teamId,'nbr'=>$nbr])];
    ...
    */
    ...
    public function addGame(Request $request)
    {
		...
		$team1 = $this->getDoctrine()->getRepository(Teams::class)->findOneBy(['teamId' =>$parametersAsArray['team_1']]);
		$team2 = $this->getDoctrine()->getRepository(Teams::class)->findOneBy(['teamId' =>$parametersAsArray['team_2']]);
		...
		$tt = $this->getDoctrine()->getRepository(MatchData::class)->findOneBy(['fPlayer'=>$parametersAsArray['player']['0']['id'],'fMatch'=>$match->getMatchId()]);
		
		
		foreach($parametersAsArray['player'] as $action)
		{
		
			$mdplayer = $this->getDoctrine()->getRepository(MatchData::class)->findOneBy(['fPlayer'=>$action['id'],'fMatch'=>$match->getMatchId()]);
			...
			$mdata = $this->getDoctrine()->getRepository(MatchData::class)->findBy(['fPlayer' =>$playerstat->getPlayerId()]);
			...
			$nbrskill = $this->getDoctrine()->getRepository(PlayersSkills::class)->findBy(['fPid' =>$playerstat->getPlayerId()]);
	...
	public function chkteam($teamId)
    {
		...
		else
		{
			...
			$positionJr = $this->getDoctrine()->getRepository(GameDataPlayers::class)->findOneBy(['fRace'=>$team->getFRace(),'qty'=>'16']);
		...
		foreach($players as $player )
		{
			...
			$skills = $this->getDoctrine()->getRepository(PlayersSkills::class)->findBy(['fPid' =>$player->getPlayerId()]);

            $nbrskill = 0;

			$nbrskill += count($skills)+$player->getAchAg()+ $player->getAchAv()+$player->getAchMa()+$player->getAchSt();
		...
		if($number<11)
		{
			...
			for($x = 1;$x<$diff+1;$x++)
			{
				...
				$players = $this->getDoctrine()->getRepository(Players::class)->findBy(['ownedByTeam'=>$team->getTeamId(),'type'=>2],['nr' => 'ASC']);
		...
		else
		{
			...
			$players = $this->getDoctrine()->getRepository(Players::class)->findBy(['ownedByTeam'=>$team->getTeamId(),'type'=>2],['nr' => 'DESC']);
